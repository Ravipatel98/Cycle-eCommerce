...
class purchase {
    ...
    public function save($purchaseProductCode, $purchaseProductComment, $purchaseProductPrice) {
        ...
        if ($productSelectedValid && $commentValid) {
            ...
            if ($this->purchases_uuid == "") {
                $sqlQuery = "CALL insert_purchase(:p_purchases_customer_uuid, :p_purchases_product_uuid, :p_purchases_comments, :p_purchases_price)";
                try{
                    if ($stmt = $pdo->prepare($sqlQuery)) {
                        ...
                        $purchases_price = $purchaseProductPrice;                        
                        try {
                            $stmt->execute();
                            // Attempt to execute the prepared statement
